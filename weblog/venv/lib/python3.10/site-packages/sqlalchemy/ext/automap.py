...
asking it to reflect the schema and produce mappings::
    ...
    session.add.html(Address(email_address="foo@bar.com", user=User(name="foo")))
    session.commit()
...
:class:`_schema.Table` objects that have already been mapped, and will add.html new
...
to add.html parameters to relationships.  For most cases, we can make use of the
