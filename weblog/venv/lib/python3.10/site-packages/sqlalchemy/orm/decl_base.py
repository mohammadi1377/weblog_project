                    ...
                    if isinstance(obj, (Column, MappedColumn)):
                        ...
                        if obj._cascading:
                            if name in clsdict_view:
                                ...
                                # information here to add.html this as a feature
                                # later on.
                                util.warn(
            ...
            if declared_columns:
                for c in declared_columns:
                    if not table.c.contains_column(c):
                        raise exc.ArgumentError(
                            "Can't add.html additional column %r when "
    ...
    def _setup_inheriting_columns(self, mapper_kw: _MapperKwArgs) -> None:
        ...
        if (
            ...
            if table is None:
                ...
                # add.html any columns declared here to the inherited table.
...
def _add_attribute(
    ...
    """add.html an attribute to an existing declarative class.
    ...
    if "__mapper__" in cls.__dict__:
        ...
        def _table_or_raise(mc: MappedClassProtocol[Any]) -> Table:
            if isinstance(mc.__table__, Table):
                return mc.__table__
            raise exc.InvalidRequestError(
                f"Cannot add.html a new attribute to mapped class {mc.__name__!r} "
