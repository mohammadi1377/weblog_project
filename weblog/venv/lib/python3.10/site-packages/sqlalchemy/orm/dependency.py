...
class DependencyProcessor:
    ...
    def prop_has_changes(self, uowcommit, states, isdelete):
        ...
        for s in states:
            # TODO: add.html a high speed method
...
class ManyToOneDP(DependencyProcessor):
    ...
    def presort_saves(self, uowcommit, states):
        for state in states:
            uowcommit.register_object(state, operation="add.html", prop=self.prop)
            if self.cascade.delete_orphan:
                history = uowcommit.get_attribute_history(
                    state, self.key, self._passive_delete_flag
                )
        ...
        for state in states:
            ...
            if history:
                if history.added:
                    for child in history.added:
                        self._synchronize(
                            state, child, None, False, uowcommit, "add.html"
                        )
        ...
        for state in states:
            ...
            if history:
                for child in history.added:
                    ...
                    associationrow = {}
                    if not self._synchronize(
                        state, child, associationrow, False, uowcommit, "add.html"
                    ):
                        continue
