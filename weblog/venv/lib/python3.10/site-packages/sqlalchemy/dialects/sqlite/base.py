...
when rendering DDL, add.html the flag ``sqlite_autoincrement=True`` to the Table
construct::
...
ABORT, FAIL, IGNORE, and REPLACE.   For example, to add.html a UNIQUE constraint
...
flag to add.html a UNIQUE constraint
...
* When using :paramref:`_sqlite.Insert.on_conflict_do_update.index_elements`
  ...
  .. sourcecode:: pycon+sql
        ...
        ...     index_elements=[my_table.c.email],
        ...     index_where=my_table.c.email.like('%@gmail.com'),
