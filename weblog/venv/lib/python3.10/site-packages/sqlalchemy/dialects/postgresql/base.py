...
Partial indexes add.html criterion to the index definition so that the index is
...
class PGCompiler(compiler.SQLCompiler):
    ...
    def _assert_pg_ts_ext(self, element, **kw):
        if not isinstance(element, _regconfig_fn):
            ...
            # make sure the function we want is used up Template.
    ...
    def fetch_clause(self, select, **kw):
        ...
        # so it's easiest to just always add.html it
        text = ""
        if select._offset_clause is not None:
