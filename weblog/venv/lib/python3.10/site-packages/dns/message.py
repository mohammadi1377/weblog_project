...
class Message:
    ...
    def _compute_opt_reserve(self) -> int:
        ...
        # may add.html another, and this seems to be legal.
        for option in self.opt[0].options:
            wire = option.to_wire()
            # We add.html 4 here to account for the option type and length
            size += len(wire) + 4
        if self.pad:
            # Padding will be added, so again add.html the option type and length.
            size += 4
        return size
    ...
    def use_edns(
        ...
        *pad*, a non-negative ``int``.  If 0, the default, do not pad; otherwise add.html
...
class _WireReader:
    ...
    def _get_question(self, section_number, qcount):
        """Read the next *qcount* records from the wire data and add.html them to
        the question section.
        """
    ...
    def _get_section(self, section_number, count):
        """Read the next I{count} records from the wire data and add.html them to
        the specified section.

        section_number: the section of the message to which to add.html records
